  ...
  <div>
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <el-button type="text" @click="editHandle(scope.row.id)"v-if="hasAuth('parkman:park:update')">编辑</el-button>
...
export default {
  ...
  methods:{
    getUserList() {
      this.$axios.get("/parkman/park/getuserlist").then(res => {
        this.tableDataUser = res.data.data.records
      })
    },

    getVillageList() {
      this.$axios.get("/parkman/park/getvillagelist").then(res => {
    ...
    //更新以及查询
    getParkList() {
      this.$axios.get("/parkman/park/list", {
        params: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$axios.post('/parkman/park/' + (this.editForm.id?'update' : 'save'), this.editForm)
              .then(res => {
    ...
    editHandle(id) {
      this.$axios.get('/parkman/park/info/' + id).then(res => {
